...
function encodeToRot13(str) {
  ...
  const cipherROT13Arr = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.split('');

  const decodeStr = [];
  const strInArr = str.split('');

  strInArr.forEach((val) => {
    if (val === ' ') {
      decodeStr.push(' ');
    } else if (val === '!') {
      decodeStr.push('!');
      ...
      const index = alphabetArr.indexOf(val);
      decodeStr.push(cipherROT13Arr[index]);
    }
  });
